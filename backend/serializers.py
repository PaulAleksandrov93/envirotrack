...
class EnvironmentalParametersSerializer(serializers.ModelSerializer):
    room = RoomSelectSerializer()
    responsible = ResponsibleSerializer(read_only=True)  # Make it read-only
    ...
    def create(self, validated_data):
        ...
        responsible = self.context['request'].user.responsible if self.context['request'].user.is_authenticated else None

        instance = EnviromentalParameters.objects.create(room=room, responsible=responsible, **validated_data)
        return instance